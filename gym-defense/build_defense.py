...
#     ・やる気の 0.2 は「やる気10で上位を占めたまま耐久を効かせられる限界」。
#       0.35まで上げるとやる気0のハピナスが3位に来て、やる気の話ではなくなる
# ══════════════════════════════════════════════════════════════════
TYPE_FIGHT_W = 1.0       # かくとう相性の重み（タイプPを4で割った段階＝二重耐性2/耐性1/弱点-2/二重弱点-3.5）
TYPE_BULK_W = 4.0        # 耐久指数の重み（いちばん硬いポケモンで耐性4つぶん）
YARUKI_MIX = 0.2         # やる気タブで耐久P・かくとう相性Pをどれだけ効かせるか（総合の1/5）
...
for e in entries:
    ...
    e['sy'] = round(e['py'] + YARUKI_MIX * (e['pb'] + e['pt']), 1)
