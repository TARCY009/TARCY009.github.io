...
(function () {
  ...
  const CRAM_SPIT = {
    // 吐き出しのダメージ＝**通常のわざと同じ式・威力45**(タイプなし＝相性もタイプ一致も掛けない)。
    // 2026-08-20タダシさん実測から逆算: ウッウ10/14/12(CP1355・攻117.24) → ガラルマッギョ
    // 0/13/13(CP1497・HP175・防127.72)へのHP満タンからの1発目が残り約84〜85%。
    // 威力45なら floor(0.5×45×(117.24/127.72)×1.3)+1 = 27(HPの15.4%)でタダシさんの見立て
    // 「約15%」と一致する。能力変化の段階は通常のわざと同じく掛かる(この吐き出し自身で入る
    // 能力ダウンはダメージ計算の**あと**に適用)。より精密な実測が出たらここを直す
    power: 45,
  ...
  function simulate(D, cfgL, cfgR, opt) {
    ...
    while (turn < maxTurn && winner === null) {
      ...
      for (const i of order) {
        ...
        let gulp = null;
        if (!shielded && o.gulp) {
          // ダメージは先に(いまの能力変化で)計算してから、能力ダウンを適用する
          const spitDmg = Math.floor(0.5 * CRAM_SPIT.power *
            (o.atk * buffMult(o.buffs[0])) / (s.def * buffMult(s.buffs[1])) * BONUS) + 1;
