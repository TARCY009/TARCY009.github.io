...
${PAGE_ROCKET ? '' : `
  ...
  <ul>
    ...
    <li>下のフレームで <b>⏸</b>（一時停止）と <b>×1</b>（倍速 ×1→×2→×4）を切り替えられます。その下の行は文字つきで、<b>⏭ コマ送り</b>＝1ターン（0.5秒）だけ進める／<b>⏩ 決断まで</b>＝次の決断の場面まで飛ばす／<b>🐢 速さ</b>＝タイムラインが流れる速さを押すたびに少しずつ遅くする（×1→×0.9→…→×0.5。×2・×4と組み合わせると掛け算になります。リアルタイム操作でも使え、結果や時計は変わりません）／<b>↺ やり直し</b>＝選んだ手を消して同じ編成でもう一度／<b>🎬 演出</b>＝カットインのON/OFF／<b>✕ 終了</b>＝バトルをやめて、ポケモンやわざを入れ替える画面に戻る、です。決着後の <b>↻</b> は同じ選択のまま再生し直します</li>
  </ul>
  <h4>わざの決め方</h4>
  <ul>
...
const paceHtml = () => `<button class="hpace${RBV.pace < 1 ? ' slow' : ''}" title="タイムラインが流れる速さ。押すたびに少しずつ遅くなります（×1→×0.9→…→×0.5→×1）。バトルの結果や時計には影響しません">🐢<b>${paceLabel()}</b></button>`;
