...
const GB_AI = {
  ...
  save:   { label: '温存',     bluff: false, save: true,  sw: false, farm: false,
    tip: '小さいSPアタックにはシールドを使いません。実際に飛んできたわざではなく「撃たれそうなわざ」をこちらのわざ構成とゲージから予測して判断します(軽いわざのブラフには引っ掛かります)。負けが見えている対面では受けてシールドを後続のために残し、逆に「戻ってくる相手への答えが控えに1匹しかない」ときは、いまの対面をシールドで確実に取りにいきます' },
  switch: { label: 'スイッチ', bluff: false, save: false, sw: true, farm: true,
    tip: '基本戦術の逃げ回り: 出し負けたらすぐ交代(ためたSPは、防がれず効く一撃のときだけ撃ってから)。勝てる対面はノーマルアタックだけで倒してゲージをため、次の相手にSPを撃ちます。倒されたら、いまの相手に有利な控えから出し直します。こちらが交代できないあいだ(クールタイム中)は五分の対面でも有利な控えを差し込み、デバフを受けたら不利・五分の対面なら交代で下げ消しします。戻ってくる相手への答えが控えに1匹しかないときは、いまの対面をシールドで確実に取りにいきます' },
...
function gbPlay(picks, foes, ans, stepwise) {
  ...
  // 控えの2匹ともが勝てるなら急所ではないので、ふつうの判断に任せる(温存なら無理に使わない)。
  ...
  const teamShieldValue = li => {
    if (!(li in shvCache)) {
      ...
      if (myBench.length && userOff.length && !aiLosing(li)) {
