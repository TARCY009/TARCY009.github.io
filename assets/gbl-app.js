...
// 交代タイマー(2026-09-13 テスターの声「あと何秒で溜まるか分かりやすく」→タダシさんが3案から選択＝案A)。
// 待っている間は交代ボタンそのものに「あと◯秒」を出し、金色が左から満ちていく。満ちきると従来どおり金色に点灯
const SWCD_HTML = `<span class="swfill"></span><span class="swcd">${SWAPMK}<small>あと</small><b></b><small>秒</small></span>`;
function setSwapCool(b, cool, left, cd) {
  b.classList.toggle('cool', cool);
  if (!cool) return;
  b.style.setProperty('--p', Math.max(0, Math.min(1, 1 - left / cd)).toFixed(3));
  const n = b.querySelector('.swcd b'); if (n) n.textContent = Math.ceil(left / 2);
}
...
// 枠に控えたノーマルアタックが「データから消えた/入っていない」か。
// **そのポケモンがノーマルアタックを1つも覚えない(マギアナ)ときは false**
// ＝作り直しても空のままなので、無限に作り直さないための歯止め
const mvGone = (id, key) => id ? !D.moves[id] : !!(key && movePool(key).fasts.length);
...
function dropUnknownPk() {
  // 戦えないポケモン(ノーマルアタックが1つも無い)も、枠に残っていたら落とす
  const ok = m => !m || (m.key && D.pokemon[m.key] && canFight(m.key));
  [S, PT, RKT, RBM, GBM, GBT, SD.my, SD.foe].forEach(store => {
    try {
      (store || []).forEach((m, i) => {
        if (!m || ok(m)) return;
        if (store === S) m.key = null;   // 1対1の左右は入れ物を作り直さず中身だけ空にする
        else store[i] = null;
      });
    } catch (e) {}
  });
...
sideEl.forEach((el, i) => {
  ...
  inp.addEventListener('input', e => {
    ...
    list.innerHTML = hits.map(k => suggRow(k, ngOf(k))).join('');
...
${PAGE_ROCKET ? '' : `
  ...
  <ul>
    <li><b>記録の中身</b> … あいての3匹（1匹目＝初手）・勝敗・自分のパーティ。
...
// ステータスは専用CPM(RCPM)方式(pvp-engine.jsのbuildStats参照)。外部シミュレーターの
...
// 2026-08-10確定: 外部シミュレーターと20例(3種別)で表示値が完全一致
...
function rbmOf(i) {
  ...
  if (!RBM[i] || RBM[i].key !== PT[i].key || !RBM[i].v || mvGone(RBM[i].fast, PT[i].key)) {
...
// ⚡=SPアタックの判断。**「＋N」(ノーマルアタックを余分に打ってから選び直す)は👊**にして区別する
// (2026-09-10タダシさん指示: ⚡にすると、ノーマルアタックのマークなのかSPのマークなのか分からなくなる)
const chipIcon = p => (p.kind === 'sh' && p.ans && p.ans.a === 'no') ? ''
  : (p.kind === 'sp' && p.ans && p.ans.a === 'wait') ? '👊' : RB_ICON[p.kind];
...
function fxShow(cls, html, dur) {
  ...
  //   「一瞬消えて、また出る」ちらつきになる。
  //   ⚠ **ちょうど80%**にする(2026-09-07タダシさん報告)。キーフレームの節目が80%なので、
  //   そこで止めれば姿勢が一致してピクッと揺れない(82%だと補間の途中で、固定値との差が揺れになる)
  setTimeout(() => el.classList.add('fxhold'), Math.round(d * 0.80));
...
function rbRender(body, bt, picks, foes, extra) {
  ...
  body.innerHTML = `<div class="rbctlbar">
    ...
    <div class="rbdock">
      ...
      ${RB.step ? `<div class="hswaprow">
        <button class="hmsw" disabled title="いつでも交代できます（押すと控えを選べます・一度交代すると45秒間は次の交代ができません）。⏭コマ送りで1ターンずつ進めながら、あいてのSPアタックが飛んでくるターンに合わせて押すと、その一撃を交代先に受けさせられます（交代受け）"><span class="swnm">${SWAPMK}<b>交代</b></span>${SWCD_HTML}</button>
      </div>
      <div class="hbtns">
  ...
  function updateHud(gt, li, hf) {
    ...
    swapEl.innerHTML = !mswBtn && swLeft > 0 ? `${SWAPMK}<b>${Math.ceil(swLeft / 2)}</b><small>秒</small>` : '';
    if (mswBtn) {
      const canSwap = RB.step && RBV.started && f.alive0 > 1 && swLeft <= 0 && gt < stop;
      mswBtn.disabled = !canSwap;
      mswBtn.classList.toggle('rdy', canSwap);
      setSwapCool(mswBtn, RBV.started && swLeft > 0 && f.alive0 > 1, swLeft, RK.swapCd);
    }
  }
  const revealTo = g => {
...
function runBlog() {
  ...
  else if (BLV.view === 'party') body.innerHTML = blPartyHtml(use);
  else if (BLV.view === 'lead') body.innerHTML = blLeadHtml(use);
...
// ---- じぶんのパーティ(2026-09-14テスター#8・タダシさん指示) ----
// 記録の mine と同じ形({k,s}の3枠)の見分け用の鍵。並び(初手)も含めて同じなら同じパーティ
const blMineKey = arr => arr && arr.some(Boolean) ? arr.map(f => f ? f.k + (f.s ? '|s' : '') : '-').join(',') : null;
const blPtMine = () => PT.map(p => p ? { k: p.key, s: !!p.shadow } : null);
const blMineNames = arr => arr.filter(Boolean).map(f => shMark(blName(f))).join('・');
// 1行のまとめ＋「最近のパーティ」(記録から自動で拾う・保存の操作は要らない)
function blRenderMine(recs) {
  ...
  rb.querySelectorAll('.blptchip').forEach(b => b.onclick = () => {
    ...
    [0, 1, 2].forEach(i => {
      const f = m[i];
      const keep = f && old.find(p => p && p.key === f.k && !!p.shadow === !!f.s);
      PT[i] = !f ? null : keep ? keep : { key: f.k, ivMode: 'auto', shadow: !!f.s, maxLv: 51 };
    });
...
// 勝率の表示(勝敗を記録したぶんだけで出す)
const blWr = (w, l) => w + l ? Math.round(w / (w + l) * 100) : null;
const blWrCell = (w, l, tip) => {
  const wr = blWr(w, l);
  return `<span class="blcell ${wr == null ? 'dim' : wr >= 50 ? 'ok' : 'bad'}" title="${tip}">${wr == null ? 'ー' : wr + '%'}<small>${w}勝${l}敗</small></span>`;
};
// パーティ別の戦績。並び(初手)が違えば別のパーティとして数える
function blPartyHtml(use) {
  const map = new Map();
  let none = 0;
  for (const r of use) {
    const key = blMineKey(r.mine);
    if (!key) { none++; continue; }
    let e = map.get(key);
    if (!e) { e = { m: r.mine, n: 0, w: 0, l: 0, last: 0 }; map.set(key, e); }
    e.n++; e.last = r.t;
    if (r.win === 'w') e.w++; else if (r.win === 'l') e.l++;
  }
  const noneNote = none ? `<div class="mtnote">自分のパーティを入れずに記録した${none}戦は数えていません</div>` : '';
  if (!map.size) return '<div class="mtnote">自分のパーティを入れて記録すると、パーティごとの戦績が出ます</div>' + noneNote;
  ...
  return `<div class="bltbl"><div class="blth blptr"><span></span><span>パーティ(左が初手)</span><span>戦数</span><span>勝率</span></div>${rows}</div>${noneNote}`;
}
// 初手: あいての初手ごとの勝率と、その裏(2・3匹目)によくいるポケモン／自分の初手ごとの勝率
function blLeadHtml(use) {
  const foe = new Map(), me = new Map();
  for (const r of use) {
    const f0 = r.foes[0];
    if (f0 && D.pokemon[f0.k]) {
      const kk = f0.k + (f0.s ? '|s' : '');
      let e = foe.get(kk);
      if (!e) { e = { f: f0, n: 0, w: 0, l: 0, back: new Map() }; foe.set(kk, e); }
      e.n++;
      if (r.win === 'w') e.w++; else if (r.win === 'l') e.l++;
      r.foes.slice(1).forEach(b => {
        if (!b || !D.pokemon[b.k]) return;
        const bk = b.k + (b.s ? '|s' : '');
        const x = e.back.get(bk) || { f: b, n: 0 };
        x.n++; e.back.set(bk, x);
      });
    }
    const m0 = r.mine && r.mine[0];
    if (m0 && D.pokemon[m0.k]) {
      const kk = m0.k + (m0.s ? '|s' : '');
      let e = me.get(kk);
      if (!e) { e = { f: m0, n: 0, w: 0, l: 0 }; me.set(kk, e); }
      e.n++;
      if (r.win === 'w') e.w++; else if (r.win === 'l') e.l++;
    }
  }
  if (!foe.size && !me.size) return '<div class="mtnote">この期間の記録がありません</div>';
  const foeRows = [...foe.values()].sort((a, b) => b.n - a.n).slice(0, 12).map((e, i) => {
  ...
  return `<div class="blsec">あいての初手<small class="expl">「裏」はその初手と一緒に出てきた2・3匹目。多い順に3匹</small></div>
    <div class="bltbl">${foeRows}</div>
    ${meRows ? `<div class="blsec">じぶんの初手</div><div class="bltbl"><div class="blth blmer"><span></span><span>ポケモン</span><span>戦数</span><span>勝率</span></div>${meRows}</div>` : ''}`;
}
...
function gbmOf(i) {
  ...
  // ⚠ ただし**ノーマルアタックを1つも覚えないポケモン**(マギアナ)では、作り直しても空のままなので、
  // 「空だから作り直す」だけにすると毎回の描画で作り直し＋保存が走り続ける(実際に踏んだ)
  if (!GBM[i] || GBM[i].key !== PT[i].key || mvGone(GBM[i].fast, PT[i].key)) {
...
// ⚠ **わざオート(MK.foeAuto)のときは、あいては枠のわざではなく環境の定番構成で戦う**。
// 選出の提案とAIの選出も同じ前提で計算しないと、**出した数字と実際のバトルが食い違う**
// (食い違い禁止ルール。2026-09-05の総点検で発見)
const sdFoeMv = m => (m && MK.foeAuto) ? sdAssumed(m) : m;
...
function sdFoePick() {
  ...
  const W = sdWinTable(pool.map(i => sdFoeMv(SD.foe[i])), targets);   // 行はpoolの並び
...
function sdSuggest() {
  ...
  const sig = JSON.stringify([my.map(i => SD.my[i]), foe.map(i => SD.foe[i]),
    MK.foeAuto, cap, cup && cup.slug, SIMOPT.buffMode]);
...
function buildSdSlots(side) {
  ...
  box.querySelectorAll('.sdslot').forEach(el => {
    ...
    inp.addEventListener('input', e => {
      ...
      list.innerHTML = hits.map(k => suggRow(k, ngOf(k,
...
function gbPlayCore(picks, foes, ans, stepwise) {
  ...
  // ---- 交代受け(2026-09-07タダシさん指示・HARDだけ) ----
  // **ユーザーのSPアタックが発動するのに合わせて交代し、その一撃を控えに受けさせる**技。
  // 実戦は先行入力が効くので、入力は発動の0.5秒前＝**1ターン前**。うちのエンジンは
  // 「交代したターンで対面を切って次の対面へ持ち越す」作りなので、
  ...
  const aiAnswerAt = (p, ctx) => {
    ...
    if (p.kind === 'swap') {
      ...
      //  ④(そろえば最適・無くてもよい)控えのだれかが、いまのユーザーのポケモンをノーマルアタックだけで倒せて、HPを6割以上残せる
      //    → いればその控えへ。いなければ形勢のいちばんマシな控えへ(下)
      ...
      if (p.w === 2) {
        ...
        if (best != null) return { a: 'toq', to: best };
        // ④を満たす控えがいなくても逃がす(2026-09-23同日タダシさん指示): 逃がしたポケモンは、のちに交代受け要員として使えるし、
        // 場面によっては重いわざまでためる機会も来る。交代先は「形勢のいちばんマシな控え」(打ち逃げ・起点回避と同じ選び方)
        const keep = aiKeepSwap(nowOv);
        return keep != null ? { a: 'toq', to: keep } : { a: 'stay' };
      }
  ...
  while (cur[0] >= 0 && cur[1] >= 0 && legs.length < 16 && !pending) {
    ...
    for (let guard = 0; guard < 90; guard++) {
      ...
      if (p.kind === 'swap') {
        ...
        // **チップも実際に交代したターンへ動かす**(時系列の順番を崩さないため)
...
function gbRender(body, bt, picks, foes) {
  ...
  bt.legs.forEach(leg => {
    ...
    rbTurns(res).forEach(t => {
      ...
      // 両者のSPが同じターンに解決した(同時発動・CMPで先後が決まる)ら、最初のSPの行の前に札を出す。
      // 決断待ちで片方の行を隠しているあいだは出さない(答えたあとに両方の行と一緒に出る)
      let simul = !partial && subs.some(r => r.ev[0] && r.ev[0].full !== undefined) && subs.some(r => r.ev[1] && r.ev[1].full !== undefined);
      for (const r of subs) {
        if (simul && ((r.ev[0] && r.ev[0].full !== undefined) || (r.ev[1] && r.ev[1].full !== undefined))) {
          items.push({ gt, html: `<div class="ft simul"><i class="tn">${first ? gt : ''}</i><i class="pmid">同時発動</i></div>` });
          first = false; simul = false;
        }
      ...
      // あいての🛡の答えも同じ(2026-09-10タダシさん指示: 答えたあとの「じぶん🛡─あいて🛡」の1枚の形で出す。
      // 答える前は「🛡ブロック」の行そのものが、あいてが使ったことを示している)
      const pl = (ptAt[t.tn] || []).filter(x => !(partial && (x.late || (x.side === 1 && x.kind === 'sh'))));
      for (let pi = 0; pi < pl.length; pi++) {
        ...
        if (q && p.kind === q.kind && p.side !== q.side) {
          ...
          // ⚠ 「同時発動」の札はここ(決断のチップ)には付けない(2026-09-10タダシさん指摘)。
          //   チップは**決めたターン**に置くので、実際に撃つターンとずれると「26Tで同時発動なのに29Tでも」と読めた。
          //   札は下の行の描画で、**両者のSPが同じターンに解決した行**に出す
          if (p.kind === 'sh'
              || (p.kind === 'sp' && firing(a0) && firing(b1)
                  && spTn[0][a0.seq] != null && spTn[0][a0.seq] === spTn[1][b1.seq])) {
            items.push(pairItem(a0, b1, gt, ''));
            pi++;
            continue;
          }
        }
  ...
  const benchAt = gt => {
    ...
    const dead = new Set();
    bt.legs.slice(0, li).forEach(l => { if (l.meDown) dead.add(l.myIdx); });
  ...
  function updateHud(gt, li, hf) {
    ...
    // じぶん側のここは従来どおり残り時間の表示だけ
    const swLeft = Math.max(0, (f.meta.swOk || 0) - ckOf(gt));
    // 交代ボタンに残り秒数を出す画面では、ボールの下の小さな秒数は出さない(同じことを2か所に出さない)
    swapEl.innerHTML = !mswBtns.length && swLeft > 0 ? `${SWAPMK}<b>${Math.ceil(swLeft / 2)}</b><small>秒</small>` : '';
    ...
    if (mswBtns.length) {
      ...
      const free = RB.step && RBV.started && swLeft <= 0 && gt < stop && !(bt.pending && gt >= stop);
      mswBtns.forEach((b, i2) => {
        const e = bench[i2];
        if (!e) { b.style.display = 'none'; b.disabled = true; return; }
        b.style.display = '';
        b.dataset.to = e.k;
        const nm = picks[e.k].name, sig = nm + (e.dead ? '!' : '');
        if (b.dataset.nm !== sig) {
          b.dataset.nm = sig;
          b.innerHTML = `<span class="swnm">${SWAPMK}<b>${shMark(nm)}</b>${tyIco(nm)}</span>${SWCD_HTML}`;
          b.title = e.dead ? `${nm} はたおれているので交代できません`
            : `${nm} にすぐ交代します（相手の打ちかけの1発は交代先に入ります・次の交代は45秒後）。` +
              `あいてのSPアタックが飛んでくるターンに合わせて押すと、その一撃を交代先に受けさせられます（交代受け）`;
        }
        const ok = free && !e.dead;
        b.disabled = !ok;
        b.classList.toggle('rdy', ok);
        b.classList.toggle('dead', !!e.dead);
        setSwapCool(b, RBV.started && swLeft > 0 && !e.dead, swLeft, GB_SWAP_CD);
      });
    }
  ...
  // ---- ⇄交代=控え2匹ぶんのボタン(2026-09-07タダシさん指示・実戦の交代ボタンの再現) ----
  ...
  function spTarget(gt, m, why) {
    ...
    // ---- 押せるタイミング(2026-09-25タダシさん指示「すべてのターンのわざで、押した瞬間に先行入力できるように」) ----
    // 表示中のターン gt の次のターン(pr+1)が「いま進んでいるターン」。実戦はノーマルアタックの**打ち始め**でゲージが増えるので、
    // **ゲージを満たすノーマルアタックを打ち始めた瞬間から**SPボタンを受け付ける(何ターンのわざでも同じ)。
    // 押したSPはその1発が当たった直後に撃つ＝先行入力。ノーマルアタックを余分に打たない。
    // ⚠ 旧(2026-09-10): 当たるターンの途中(表示中が E-1)からしか押せず、2ターン以上のわざでは受付が遅れていた
    //   (1ターンわざは打ち始め・2ターンわざは0.5秒後・3ターンわざは1.0秒後)。実戦と違い、入力が遅れる原因になっていた
    const tl = leg._tl || (leg._tl = rbTurns(leg.res));
    const fm = D.moves[leg.pol.fast], tnMe = fm && fm.tn ? fm.tn : 1;
    ...
    if (enNow >= m.e) {
      ...
      // この1発でゲージが足りる: その1発を打ち始めていれば(いま進んでいるターン pr+1 が打ち始め以降なら)押せる
      if (!next) return no('この対面のうちに切れ目が来ない');
      if (why) why.push('next=' + next.tn + '/' + next.state[0].en + ' now=' + enNow + ' tn=' + tnMe);   // 確認用(spProbe)
      if (next.state[0].en < m.e) return no('ゲージ不足(' + next.state[0].en + '<' + m.e + ')');
      if (next.tn - pr > tnMe) return no('まだ早い(その1発を打ち始めるのは' + (next.tn - tnMe + 1 - pr) + 'ターン先)');
      on = next.tn + 1;
    }
  ...
  RBV.spProbe = (gt, id) => { const w = []; const r = spTarget(gt, D.moves[id], w); return (r ? 'ok:' + r.on + ' ' : '') + (w.join(',') || (r ? '' : '不明')); };
...
function fillMoves(i, cfg) {
  ...
  // (シャドウ補正をかける前の値＝ゲームやシミュレーターの表示に合わせる)
...
(function init() {
  ...
  bootStep('対戦記録', () => {
  buildBlogSlots();
  buildPartySlots(document.querySelector('#blog .blmyslots'));   // じぶんのパーティ(PTを共有・わざ欄なし)
  const myEdit = document.querySelector('#blog .blmyedit');
  if (myEdit) myEdit.onclick = () => {
    const sl = document.querySelector('#blog .blmyslots');
    sl.hidden = !sl.hidden;
