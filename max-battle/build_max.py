...
MANUAL_INCLUDE_D = [
    "DEINO", "ZWEILOUS", "HYDREIGON",   # サザンドラ系統 (2026-07-19実装、GM未反映)
...
def main():
    ...
    def settings_to_entry(tid, ps, cat, fixed=None, gname=None):
        ...
        for q in (ps.get("quickMoves", []) or []):
            ...
            if ty in T_IDX:
                mj = gym["moves"].get(q, {})
                fasts.append({"jp": mj.get("jp", q), "t": T_IDX[ty], "e": 0})
        for q in (ps.get("eliteQuickMove", []) or []):
            ty = move_type.get(q)
            if ty in T_IDX:
                mj = gym["moves"].get(q, {})
                fasts.append({"jp": mj.get("jp", q), "t": T_IDX[ty], "e": 1})
        entry = {"n": name, "dex": dex, "ty": types_idx, "atk": atk, "cat": cat}
        if cat == "D":
            entry["fm"] = fasts
        else:
    ...
    for pid in MANUAL_INCLUDE_D:
        ...
        for tid, ps in poke.items():
            if ps.get("pokemonId") == pid and (
                tid.endswith("_NORMAL") or
                re.match(r"^V\d{4}_POKEMON_" + re.escape(pid) + r"$", tid)):
                cand = (tid, ps)
